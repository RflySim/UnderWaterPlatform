...
REM UUVModel uses ClassID 600, matching Submarine2.xml.
SET /a CLASS_3D_ID=600


REM Set the start UDP port for SIMULINK/OFFBOARD API
REM Kept for compatibility notes; RflySim v4.12 CopterSim no longer takes this as the third argument.
...
REM E.g., fixed-wing aircraft: PX4SitlFrame=plane; small cars: PX4SitlFrame=rover; UUV: uuv_bluerov2_heavy  uuv_hippocampus
set PX4SitlFrame=uuv_bluerov2_heavy


REM Set the map, use index or name of the map on CopterSim
REM e.g., UE4_MAP=1 equals to UE4_MAP=Grasslands RealForestBLake
...
REM UE5Path
cd /d %PSP_PATH%\RflySimUE5
tasklist|find /i "RflySim3D.exe" || start %PSP_PATH%\RflySimUE5\RflySim3D.exe -key=I1
...
start /realtime CopterSim.exe 1 %cntr% %CLASS_3D_ID% %DLLModel% %SimMode% %UE4_MAP% %IS_BROADCAST% %PosXX% %PosYY% %ORIGIN_YAW% 1 %UDPSIMMODE% %IsSysID% %ORIGIN_POS_Z%,%ORIGIN_ROLL%,%ORIGIN_PITCH%
